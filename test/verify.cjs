...
(async () => {
  ...
  check('brand: renamed to BeanBeanDragon', (await page.evaluate(() => document.title)).includes('BeanBeanDragon'));
  ...
  check('anti-fake: code format BBD-', /^BBD-[A-Z0-9]+-\d{2}$/.test((await page.locator('.fake-code-row .fake-code').textContent() || '').trim()));
  ...
  await page.fill('#fakeCodeInput', 'BBD-NOTEXIST-00');
