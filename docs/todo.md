...
## 十三之二、2026-09-11 全站美术重制（AI 原始出图标准）
...
## 十五、2026-09-16 待决策：Email Sending 需 $5/月（Workers Paid）—— 已冻结

### 背景
- 2026-09-11：邮件链路代码已全部接通并部署（`workers/mailer` + Pages service binding），
  线上实测只差最后一步，报错为 `E_SENDER_DOMAIN_NOT_CONFIGURED`。
- 2026-09-16：用户反馈在控制台开通 Email Sending **需要 $5/月**（即 Workers Paid 套餐）。

### 决定（用户 2026-09-16）
- [ ] **暂不开通，计入待办**。用户预告接下来有**重大更新，可能要大修**，
      在此之前冻结下面这些动作，等重大更新的方向明确后再一起决定：
  - 开通 Workers Paid / Email Sending（含 Onboard Domain）
  - 启用 R2（附件 / 名片上传）
  - 前端 `api.js` 从 `mock` 切到 `http` 模式
  - 相关的部署与推送
- 冻结范围只限上面这些；**与重大更新无关的缺陷修复、文档整理照常进行**。

### 恢复后要做的
- [ ] 付费后：Compute → Email Service → Email Sending → Onboard Domain → 选 `beanbeanmouse.com`
      （自动写入 SPF / DKIM / DMARC，DNS 一般 5–15 分钟生效）。
- [ ] 我复测真实发信（注册 / 重发验证邮件），确认 `mail_outbox.status = 'sent'`。
- [ ] 同批启用 R2，并取消根 `wrangler.jsonc` 里 R2 绑定的注释、创建 `beanbeanmouse-files` 桶。

### 备选方案（不花钱）
- **Resend 免费额度 3000 封/月（每天 100 封）**：Pages 侧的通用 HTTP 发信通道已就绪
  （`MAIL_TRANSPORT=http` + `MAIL_API_URL` / `MAIL_API_KEY`），接口形状与 Resend 完全一致，
  改成配置即可切换，无需改代码。适合"先不付费、但要把注册→验证→登录闭环验通"的情况。

### 参考
- `workers/mailer/README.md`、`docs/deploy-backend.md`（邮件链路与密钥清单）

## 十六、2026-09-16 pet0.2：改为宠物用品自营外贸站

### 定位变更（用户 2026-09-16 确认）
- 从"综合外贸平台（多品类 + 买卖双方撮合）"改为**宠物用品自营外贸站**：平台自己卖货，
  用户向平台询问；商家注册改为**供货 / 托管合作申请**（我们承担风险与库存，按约定分成）。
- 删除"卖家主页""买卖双方互评""佣金撮合"；旧站整站归档到 `archive/general-trade-v0.2/`
  （含 README，记入未完成 idea，外贸流程模块后续按需搬回）。

### 已完成
- [x] 旧站归档 + 版本号改 **pet0.2**（`package.json` 用合法的 `0.2.0-pet.1`，页脚显示 pet0.2）。
- [x] 数据层：品类树改为宠物 8 细分（仓鼠小宠 / 猫 / 小型犬 / 大型犬 / 食品零食 / 美容清洁 /
      玩具训练 / 出行智能）；卖家收敛为平台自营单主体；商品 32 款（含 22 款新概念商品），
      全部带 `pets / petSize / material` 结构化字段；商品简介已核验 ≤300 字符。
- [x] 新门面首页（`#/`）：趴桌仓鼠主视觉 + 暖色调 + 搜索 + 三个 CTA + 4 条信任标签 +
      8 细分入口 + 精选商品 + 视频墙预告 + 服务承诺 + 关于我们预告 + 询问 CTA。
- [x] 新增 `#/videos` 视频墙（客户投稿精选；支持 B站/YouTube/抖音 外链 + 来源与授权声明）
      与 `#/about` 关于我们（半页吉祥物 + 半页联系方式，联系方式待补，邮箱已可用）。
- [x] 注册改双模块：**客户（我要买）** / **商家（供货·托管合作）**，商家侧列出 6 项
      **必交资质清单**（营业执照、检测报告、商标授权、生产许可、工厂实拍、出口结算主体）。
- [x] 文案改口径：询盘入口改为"向豆豆鼠询问"，认证标签改"自营 · 已核验"。
- [x] 单行省略工具类 `.oneline`（超长用 … 省略，`title` 悬浮显示完整内容），已用于视频卡。

### 待办
- [ ] **AI 素材**：门面主视觉、关于我们、8 个细分像素图标、8 张概念商品图（生成中，见 `work/gen-pet-assets.mjs`）；
      商品图映射接入 `product-image-map.js`。
- [ ] **像素循环动画**：按"断网小恐龙"风格重做 3 段 5 秒无缝循环（陆/海/空），模型用默认
      `doubao-seedance-2-0-fast-260128`；生成后走 `work/make-loop2.mjs` 循环窗口搜索 + 交叉淡化。
- [ ] **对话框"奇怪字符"**：用户反馈对话框有符号占位。已定位两个可疑源：大量 `·` 分隔符、
      翻译胶囊里的 `⚡` 与卖家信息里的国旗 emoji（部分 Windows 字体渲染成方块）。需截图确认后统一替换。
- [ ] **文字换行**：语言切换后标题/标签变两三行 → 统一单行 + 省略号 + 悬浮完整（`.oneline` 需铺开到商品卡/聊天列表）。
- [ ] **商业纠纷与网络风险**（用户 2026-09-16 特别要求）：
  - 视频墙 UGC 授权与转载声明（已加基础声明，仍需法律措辞审定）；
  - 商品图/AI 概念图不得暗示未经授权的品牌与认证；
  - 商家资质仅用于审核，存储与展示需注意个人信息与商业秘密（当前 D1 明文，后续加密/隔离）；
  - 询盘/报价记录留存与举证能力（订单留痕已具备链式证据）。
- [ ] 后端 seed 的演示商品仍是综合品类（p1/p2），切 http 模式前需改成宠物商品。
- [ ] **邮件通道：等用户后续自行开通，暂不投入**（2026-09-17 用户确认）。
      代码侧已全部就绪（`workers/mailer` + Pages service binding + 品牌化验证邮件模板 + 失败原因落库），
      只差域名 Email Sending 开通那一步；开通后重新注册即可生效，无需改代码。详见第十五节。
